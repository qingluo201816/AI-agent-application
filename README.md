...
| 模块 | 路径 | 作用 |
| --- | --- | --- |
| 主包 | `com.qingluo.writeaiagent` | 后端代码根命名空间，组织所有业务模块。 |
| Controller | `com.qingluo.writeaiagent.controller` | 提供 HTTP 接口（健康检查、AI 对话入口等）。 |
| Agent | `com.qingluo.writeaiagent.agent` | 智能体推理、工具编排、任务执行核心。 |
| Agent Model | `com.qingluo.writeaiagent.agent.model` | 智能体运行状态、上下文等结构定义。 |
| App | `com.qingluo.writeaiagent.app` | 面向业务场景的应用封装（如写作智能体）。 |
| Advisor | `com.qingluo.writeaiagent.advisor` | 会话增强能力（日志、重读等）。 |
| RAG | `com.qingluo.writeaiagent.rag` | 知识库加载、分片、检索与查询增强。 |
| Tools | `com.qingluo.writeaiagent.tools` | Agent 可调用的外部工具（搜索/抓取/文件/PDF 等）。 |
| ChatMemory | `com.qingluo.writeaiagent.chatmemory` | 对话历史持久化与多轮上下文管理。 |
| Config | `com.qingluo.writeaiagent.config` | 全局配置（CORS、Bean 装配等）。 |
| Constant | `com.qingluo.writeaiagent.constant` | 项目共享常量定义。 |
| Demo Invoke | `com.qingluo.writeaiagent.demo.invoke` | 多种模型接入方式示例。 |
| Demo RAG | `com.qingluo.writeaiagent.demo.rag` | RAG 实验与示例代码。 |
| MCP Server | `com.qingluo.writeimagesearchmcpserver` | 独立图像搜索 MCP 服务。 |
| MCP Tools | `com.qingluo.writeimagesearchmcpserver.tools` | MCP 图像搜索工具实现。 |
...
### 1）主工程 `src/main/java/com/qingluo/writeaiagent`

#### `com.qingluo.writeaiagent`
- `WriteAiAgentApplication`：Spring Boot 启动入口，负责拉起后端应用。
- `package-info.java`：主包说明，定义该层作为业务聚合根。

#### `com.qingluo.writeaiagent.controller`
...
#### `com.qingluo.writeaiagent.app`
- `WriteApp`：写作智能体应用服务层，封装 ChatClient、会话记忆、RAG、Tool Calling、MCP 调用能力。
- `package-info.java`：应用层职责说明（面向业务场景编排）。

#### `com.qingluo.writeaiagent.agent`
- `BaseAgent`：智能体抽象基类，定义状态、步骤推进、流式输出等通用能力。
- `ToolCallAgent`：支持工具调用的智能体实现，负责将工具执行纳入推理循环。
- `ReActAgent`：ReAct 模式智能体实现（思考/行动迭代）。
- `WriteManus`：项目默认“超级智能体”实现，预置系统提示词、最大步骤数与日志 Advisor。
- `package-info.java`：Agent 分层说明。

#### `com.qingluo.writeaiagent.agent.model`
- `AgentState`：智能体运行态模型（目标、历史步骤、当前状态等）。
- `package-info.java`：模型层说明。

#### `com.qingluo.writeaiagent.advisor`
...
#### `com.qingluo.writeaiagent.chatmemory`
- `FileBasedChatMemory`：基于文件持久化的会话记忆实现（适合本地开发演示）。
- `package-info.java`：记忆模块说明。

#### `com.qingluo.writeaiagent.rag`
- `WriteAppDocumentLoader`：加载小说写作知识文档（本地 Markdown 等）并供向量化入库。
...
- `WriteAppVectorStoreConfig`：向量存储 Bean 配置（小说写作应用知识库）。
- `PgVectorVectorStoreConfig`：PgVector 向量库配置（数据库向量存储能力）。
- `WriteAppContextualQueryAugmenterFactory`：上下文查询增强器工厂，组装检索上下文增强策略。
- `WriteAppRagCustomAdvisorFactory`：自定义 RAG Advisor 工厂，拼装检索问答链路。
- `WriteAppRagCloudAdvisorConfig`：云端知识库 RAG Advisor 配置。
- `package-info.java`：RAG 模块职责说明。

#### `com.qingluo.writeaiagent.tools`
...
#### `com.qingluo.writeaiagent.config`
- `CorsConfig`：跨域配置，支持前端本地调试。
- `package-info.java`：配置层说明。

#### `com.qingluo.writeaiagent.constant`
- `FileConstant`：文件路径/目录等常量。
- `package-info.java`：常量层说明。

#### `com.qingluo.writeaiagent.demo.invoke`（教学示例）
...
#### `com.qingluo.writeaiagent.demo.rag`（教学示例）
- `MultiQueryExpanderDemo`：多查询扩展检索实验示例。
- `package-info.java`：RAG 示例层说明。

### 2）独立 MCP 服务工程 `write-image-search-mcp-server/src/main/java/com/qingluo/writeimagesearchmcpserver`

#### `com.qingluo.writeimagesearchmcpserver`
- `WriteImageSearchMcpServerApplication`：图片搜索 MCP 服务启动入口。
- `package-info.java`：MCP 服务主包说明。

#### `com.qingluo.writeimagesearchmcpserver.tools`
...
```mermaid
flowchart LR
    A[Vue Frontend<br/>:3000] -->|SSE /api/ai/write_app/chat/sse| B[AiController]
...
> 典型链路 1（写作智能体）：`Frontend -> /ai/write_app/chat/sse -> AiController -> WriteApp -> ChatClient(记忆/Advisor/RAG/Tools/MCP)`。
>
> 典型链路 2（超级智能体）：`Frontend -> /ai/manus/chat -> AiController -> WriteManus -> ToolCallAgent 循环 -> 本地 Tool 或 MCP Tool`。
...
1. **启动 MCP 服务（图片搜索）**
   - 目录：`write-image-search-mcp-server`
...
4. **联调自检（建议）**
   ...
   - 写作智能体 SSE：`GET http://localhost:8123/api/ai/write_app/chat/sse?message=你好&chatId=test1`
